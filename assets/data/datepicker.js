...
class Datepicker{
    constructor(host,s){
        ...
        document.addEventListener("click",e=>{
            if(e.target==document.getElementById("datepicker")&&!document.getElementById("datepicker-frame")){t.load("day");show(true);}
        ...
        t.load=function(n){
            ...
            if(n=="day"){
                ...
                }else prev.className="disabled";const head=document.createElement("th");
                row1.append(head);
                head.colSpan=5;
                head.innerHTML=months[t.date.getMonth()]+" "+t.date.getFullYear();
                head.onclick=()=>{t.load("month");};
                head.className="pointer";const next=document.createElement("th");
                row1.append(next);next.innerHTML=">>";
                ...
                t.table.append(row);
                const prev=document.createElement("th");
                row.append(prev);prev.innerHTML="<<";
                if(t.firstdate==undefined||(t.date.getFullYear()>t.firstdate.getFullYear())){
                    prev.className="pointer";
                    prev.onclick=()=>{t.date=new Date(t.date.getFullYear()-1,1,1);
                        t.load("month");
                    };
                ...
                row.append(next);next.innerHTML=">>";
    ...
    config(s){
        this.firstdate=s.firstdate||this.firstdate;this.lastdate=s.lastdate||this.lastdate;this.disableddays=s.disableddays||this.disableddays||(()=>{return true;});this.format=s.format||this.format||((d)=>{return d;});if(typeof this.firstdate!="object"&&this.firstdate!=undefined)console.error("firstdate is not of type Object");else if(typeof this.lastdate!="object"&&this.lastdate!=undefined)console.error("lastdate is not of type Object");else if(typeof this.disableddays!="function")console.error("disableddays is not of type function");else if(typeof this.format!="function")console.error("format is not of type function");
        const d=new Date();
        let date=d;
        ...
        this.date=this.date||date;this.host.value=this.format(this.date);
    }
    ...
    setDate(date){
        ...
        this.date=date;this.host.value=this.format(date);
